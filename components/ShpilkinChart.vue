...
function prepareCanvas(element: HTMLCanvasElement, context: CanvasRenderingContext2D): void {
  const ratio = Math.max(1, (window.devicePixelRatio || 1) * element.clientWidth / canvasSize)
  const backingSize = Math.round(canvasSize * ratio)
...
const pointRadius = (point: Point) => {
  const cssScale = canvasSize / (canvas.value?.clientWidth || canvasSize)
  return Math.min(4, 0.5 + Math.log2(Math.max(1, point.count)) * 0.5) * cssScale
}
...
function draw(): void {
  ...
  prepareCanvas(element, context)
  const width = canvasSize; const height = canvasSize; const left = 48; const right = 16; const top = 14; const bottom = 34; const maxShare = 100
...
function nearest(event: MouseEvent): void {
  ...
  for (const point of nearby) { const px = left + point.turnout / 100 * (canvasSize - left - right); const py = canvasSize - bottom - point.share / 100 * (canvasSize - top - bottom); const next = (px - x) ** 2 + (py - y) ** 2; if (next < distance) { hit = point; distance = next; hitX = px; hitY = py } }
...
function drawAbsolute(): void {
  ...
  prepareCanvas(element, context)
  const width = canvasSize; const height = canvasSize; const left = 48; const right = 16; const top = 14; const bottom = 34; const maxVotesValue = maxVotes.value
  context.clearRect(0, 0, width, height); context.font = '11px Arial, sans-serif'; context.strokeStyle = '#eef2f7'; context.fillStyle = '#697586'
...
function drawCrosshair(element: HTMLCanvasElement | undefined, hover: { x: number, y: number } | null, maxY: number, yFormatter: (value: number) => string): void {
  ...
  prepareCanvas(overlay, context)
  const width = canvasSize; const height = canvasSize; const left = 48; const right = 16; const top = 14; const bottom = 34; context.clearRect(0, 0, width, height); if (!hover) return
...
function absoluteMove(event: MouseEvent): void {
  ...
  for (const series of absoluteSeries.value) for (const point of series.points) { const px = left + point.turnout / 100 * (canvasSize - left - right); const py = canvasSize - bottom - point.votes / maxVotes.value * (canvasSize - top - bottom); const next = (px - x) ** 2 + (py - y) ** 2; if (next < distance) { distance = next; snapX = px; snapY = py } }
