...
function draw(): void {
  ...
  const maxShare = 100; const color = entities.value[entityIndex.value]?.color || palette[0]
  ...
  for (const point of points.value) { const x = left + Math.min(100, Math.max(0, point.turnout)) / 100 * (width - left - right); const y = height - bottom - point.share / maxShare * (height - top - bottom); context.globalAlpha = 1 - Math.pow(1 - .08, point.count); context.beginPath(); context.arc(x, y, 1.5, 0, Math.PI * 2); context.fill() }
  context.globalAlpha = 1
}
function nearest(event: MouseEvent): void {
  const element = canvas.value; if (!element || !points.value.length) return
  const box = element.getBoundingClientRect(); const x = (event.clientX - box.left) / box.width * element.width; const y = (event.clientY - box.top) / box.height * element.height; const maxShare = 100; const left = 48; const right = 16; const top = 14; const bottom = 34
...
<template><section class="shpilkin-chart"><div class="chart-head"><div><span class="eyebrow">АНАЛИЗ УИК</span><h3>Метод Шпилькина</h3></div><select v-model="selected"><option v-for="entity in entities" :key="entity.id" :value="entity.id">{{ entity.name }}</option></select></div><p class="chart-title">Результат «{{ entities[entityIndex]?.name }}»</p><p class="chart-subtitle">Явка (%) / доля голосов (%) — облако УИК.</p><canvas ref="canvas" width="960" height="420" @mousemove="nearest" @mouseleave="tooltip = ''" /><p class="chart-note">{{ tooltip || 'Точки показывают реальные протоколы УИК.' }}</p><p class="chart-title">Суммарные голоса по явке</p><p class="chart-subtitle">Явка (%) / абсолютное число голосов. Интервал агрегации — 0,1%.</p><canvas ref="absoluteCanvas" width="960" height="320" /><div class="chart-legend"><span v-for="entity in entities" :key="entity.id"><i :style="{ background: entity.color }" />{{ entity.name }}</span></div></section></template>
